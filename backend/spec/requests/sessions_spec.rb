...
RSpec.describe "Sessions" do
  describe "POST /api/sessions" do
    it "logs in a verified user and sets the auth cookie" do
      ...
      post "/api/sessions", params: { email: user.email, password: "secret123" }, as: :json
      ...
      expect(response.parsed_body["user"]).not_to have_key("verificationToken")
      expect(response.parsed_body["user"]).not_to have_key("jobBoardEntriesCount")
      expect(response.parsed_body["token"]).to be_present
      expect(response.parsed_body["expiresAt"]).to be_present
    ...
    it "rejects invalid credentials" do
      ...
      post "/api/sessions", params: { email: user.email, password: "wrong" }, as: :json
    ...
    it "requires email and password" do
      post "/api/sessions", params: { email: "a@b.com" }, as: :json
  ...
  describe "POST /api/sessions/register" do
  ...
  describe "GET /api/sessions/verify" do
    it "verifies a valid token" do
      ...
      get "/api/sessions/verify", params: { token: user.verification_token }
  ...
  describe "GET /api/sessions/me" do
    it "returns the current user from the auth cookie" do
      ...
      get "/api/sessions/me", headers: { "Cookie" => "auth_token=#{session_record.token}" }
  ...
  describe "DELETE /api/sessions" do
    it "destroys the session and clears the cookie" do
      ...
      delete "/api/sessions", headers: { "Cookie" => "auth_token=#{session_record.token}" }
