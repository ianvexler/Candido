...
RSpec.describe "Users" do
  describe "PUT /api/users" do
    it "updates setupCompleted for the current user" do
      ...
      expect(response.parsed_body["user"]).to include("setupCompleted" => true, "email" => user.email)
  ...
  describe "GET /api/users" do
    it "returns users with job board counts for admins" do
      ...
      expect(listed["jobBoardEntriesCount"]).to eq(2)
      expect(listed).not_to have_key("password")
    end

    it "forbids non-admins" do
      get "/api/users", headers: auth_headers(create(:user))
